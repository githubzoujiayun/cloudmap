...
/*
 * 测试启动另一个activity时候，原有activity自杀(在listviewactivity中的onPause方法中调用this.finish()方法)
 * 变相解决编辑和删除listview条目后的刷新问题
 */
public class CloudMapActivity extends Activity {
	...
	private ArcGISLocalTiledLayer dltb0Layer;
	private ArcGISLocalTiledLayer dltbLayer; // 乡镇地类
	...
	private void importLayers() {
		...
		// 加载地类图斑(全县)
		String path0 = tpkPath + "dltb0.tpk";
		dltb0Layer = new ArcGISLocalTiledLayer(path0);
		map.addLayer(dltb0Layer);
		// 加载谷歌地图
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.dltb:
			...
			if (map.isLoaded()) {
				if (dltb0Layer.isVisible()) {
					dltb0Layer.setVisible(false);
				} else {
					dltb0Layer.setVisible(true);
				}
		...
		case R.id.backupDatabase: // 备份数据库菜单
			...
			bdBuilder.setPositiveButton("确认", new OnClickListener() {
				@Override
				public void onClick(DialogInterface dialog, int which) {
					new BackupTask(getApplicationContext()).execute("backupDatabase");
